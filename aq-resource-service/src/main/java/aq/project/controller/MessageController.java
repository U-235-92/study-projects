...
import aq.project.service.MessageService;
...
public class MessageController {
	...
	private final MessageService resourceService;
	
	@ResponseBody
	@PostMapping("/write-message")
	@ResponseStatus(code = HttpStatus.CREATED)
	public String writeMessage(@RequestBody(required = true) MessageRequest messageRequest) {
		resourceService.writeMessage(messageRequest);
		return String.format("Message from author %s was written", messageRequest.getAuthor());
	}
	...
	public MessageResponse readMessage(@PathVariable(required = true) int id) {
		return resourceService.readMessage(id);
	}
	...
	public String editMessage(@PathVariable(required = true) int id, @RequestBody(required = true) String text) {
		resourceService.editMessage(id, text);
		return String.format("Message with id %d was edited", id);
	}
	...
	public String deleteMessage(@PathVariable(required = true) int id) {
		resourceService.deleteMessage(id);
		return String.format("Message with id %d was deleted", id);
	}
}
