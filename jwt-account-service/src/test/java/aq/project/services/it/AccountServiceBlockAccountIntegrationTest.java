...
class AccountServiceBlockAccountIntegrationTest {
	...
	private void initRepo() {
		Account junkoAccount = new Account(AccountLogins.JUNKO, passwordEncoder.encode("58"), true, Role.READER);
		when(accountRepository.findByLogin(AccountLogins.JUNKO)).thenReturn(Optional.of(junkoAccount));
	}
	
	@Test
	@DisplayName("success-block-account-test")
	void test1() {
	...
	@Test
	@DisplayName("fail-null-account-block-test")
	void test2() {
		assertThrows(NullPointerException.class, () -> accountService.blockAccount(null));
	}
	
	@Test
	@DisplayName("fail-not-found-account-block-test")
	void test3() {
